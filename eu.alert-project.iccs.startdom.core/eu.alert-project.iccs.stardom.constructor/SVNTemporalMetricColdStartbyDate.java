...
public class SVNTemporalMetricColdStartbyDate {
				...
				if (filerevisions != null) {
					for (Iterator revisionentries = filerevisions.iterator(); revisionentries
						...
						if (date != Calendar.getInstance().getTime()
								&& author != null) {
							System.out.println(author + " changed file: " + "/"
									+ (path.equals("") ? "" : path + "/")
									+ entry.getName() + " revision: "
									+ revisionEntry.getRevision() + " on "
									+ date.toString());
						}
					}
				}
				System.out.println(entry.getAuthor() + " changed file: " + "/"
						+ (path.equals("") ? "" : path + "/") + entry.getName()
						+ " revision: " + entry.getRevision() + " on "
